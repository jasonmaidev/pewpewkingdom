	...
	Module.XMLHttpRequest = (function () {
		...
		function UnityCache() {
			...
			try {
				...
				openRequest.onupgradeneeded = function (e) {
					var objectStore = e.target.result.createObjectStore(XMLHttpRequestStore.name, {
						keyPath: 'url'
					});
			...
			if (this.database) {
				try {
					var target = this.database
						.transaction(
							[store],
							['put', 'delete', 'clear'].indexOf(operation) != -1 ? 'readwrite' : 'readonly'
						)
						.objectStore(store);
					...
					if (cache.control == 'immutable') {
						...
						if (cache.result.responseHeaders['Last-Modified']) {
							xhr.setRequestHeader(
								'If-Modified-Since',
								cache.result.responseHeaders['Last-Modified']
							);
